...
import { getSupervisorProfile } from '../../lib/supervisorApi'

const NAV_ITEMS = [
  { to: '/supervisor', end: true, icon: HomeIcon, label: 'Dashboard' },
  { to: '/supervisor/opportunities', icon: Briefcase, label: 'Opportunities' },
  { to: '/supervisor/applications', icon: FileText, label: 'Applications' },
  { to: '/supervisor/profile', icon: UserRound, label: 'Profile' },
]
...
export default function SupervisorSidebar({ open, onClose }) {
  ...
  const fullName = React.useMemo(() => {
    const f = (profile?.first_name || '').trim()
    const l = (profile?.last_name || '').trim()
    return [f, l].filter(Boolean).join(' ') || 'Supervisor'
  }, [profile])
    ...
    <>
      ...
      >
        <div className="flex items-start justify-between px-2.5 mb-4">
          <Link to="/supervisor" className="block">
        ...
        <div className="mt-auto px-1">
          ...
          <div className="flex items-center gap-2.5 px-2 py-2">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[11px] text-ink-3">Supervisor</div>
            </div>
          </div>
          <button
            type="button"
